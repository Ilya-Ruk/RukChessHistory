...
/*
	256 (half feature output dimension) x 16 bits (I16) = 4096 bits

	4096 bits / 256 bits (AVX2 register size) = 16 registers needed
*/
#define NUM_REGS						16
...
#define NNUE_FILE_LENGTH				21022697
...
_declspec(align(64)) I16 InputWeights[HALF_FEATURE_OUTPUT_DIMENSION * HALF_FEATURE_INPUT_DIMENSION]; // 256 x 41024 = 10502144
...
{
	__m128i* BiesesTile = (__m128i*)Biases;
	...
	memcpy(BiesesTile, Temp, 8 * sizeof(__m128i));
}
#endif // USE_NNUE_AVX2
...
//	printf("Architecture length = %d\n", ArcLength);
	...
	if (fgets(Architecture, ArcLength + 1, File) == NULL) {
		printf("File '%s' read error!\n", NNUE_FILE);
...
//	printf("Header 1 = 0x%X\n", Header1);

	if (Header1 != NNUE_HEADER_1) {
		printf("File '%s' read error!\n", NNUE_FILE);
...
//		printf("InputBiases[%d] = %d\n", Index, InputBiases[Index]);
	...
	for (int Index = 0; Index < HALF_FEATURE_OUTPUT_DIMENSION * HALF_FEATURE_INPUT_DIMENSION; ++Index) { // 256 x 41024 = 10502144
...
//	printf("Header 2 = 0x%X\n", Header2);

	if (Header2 != NNUE_HEADER_2) {
		printf("File '%s' read error!\n", NNUE_FILE);
...
BOOL RefreshAccumulator(BoardItem* Board)
{
	HistoryItem* Info;
	...
	Info = &Board->MoveTable[Board->HalfMoveNumber - 1];
