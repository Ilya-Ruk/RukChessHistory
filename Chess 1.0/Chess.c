...
BOOL IsInsufficientMaterial(BoardItem *Board)
{
	int AllCount = 0;
	...
	for (int i = 0; i < 16; i++) {
		if (Board->WhitePieceList[i] == -1) {
			continue;
		}

		AllCount++;

		if (Board->Pieces[Board->WhitePieceList[i]] == PAWN) {
			PawnCount++;
		}
		else if (Board->Pieces[Board->WhitePieceList[i]] == ROOK) {
			RookCount++;
		}
		else if (Board->Pieces[Board->WhitePieceList[i]] == QUEEN) {
			QueenCount++;
		}
	}

	for (int i = 0; i < 16; i++) {
		if (Board->BlackPieceList[i] == -1) {
			continue;
		}

		AllCount++;

		if (Board->Pieces[Board->BlackPieceList[i]] == PAWN) {
			PawnCount++;
		}
		else if (Board->Pieces[Board->BlackPieceList[i]] == ROOK) {
			RookCount++;
		}
		else if (Board->Pieces[Board->BlackPieceList[i]] == QUEEN) {
			QueenCount++;
		}
	}
...
int QuiescenceSearch(BoardItem *Board, int Alpha, int Beta, const int Depth, const int Ply, MoveItem *BestMoves, const BOOL IsPrincipal, const BOOL InCheck) // Поиск хода и возврат оценки
{
  ...
  if (IsInsufficientMaterial(Board) || PositionRepeat1(Board) || Board->FiftyMove >= 100) { // Повтор позиции
    return 0;
  }
...
int Search(BoardItem *Board, int Alpha, int Beta, int Depth, const int Ply, MoveItem *BestMoves, const BOOL IsPrincipal, const BOOL InCheck, const BOOL IsNull) // Поиск хода и возврат оценки
{
  ...
  if (Ply > 0 && (IsInsufficientMaterial(Board) || PositionRepeat1(Board) || Board->FiftyMove >= 100)) { // Не корень дерева и повтор позиции
    return 0;
  }
...
void Game(const int HumanColor, const int ComputerColor) // Играем партию
{
  ...
  MaxTimeForMove = (MaxInputTimeForMove >= 1 && MaxInputTimeForMove <= MAX_TIME) ? (U64)MaxInputTimeForMove : (U64)MAX_TIME;
  MaxTimeForMove *= (U64)1000;

  printf("Max. threads (0-%d): ", ComputerMaxThreads);
  scanf("%d", &MaxInputThreads);

  MaxThreads = (MaxInputThreads >= 1 && MaxInputThreads <= ComputerMaxThreads) ? MaxInputThreads : 1;
